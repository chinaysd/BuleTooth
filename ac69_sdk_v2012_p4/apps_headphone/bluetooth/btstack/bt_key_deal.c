...
{
	return 0;//0:set sys_vol auto, 1:do nothing with sys_vol
}
...
#if  USE_HTK
   #if TONE_AFTER_HTK_START
			if(user_val->ring_htk_start_flag==1)
			{
			    user_val->ring_htk_start_flag =0;
                htk_start(1);//播完提示音再启动语言识别，type参数为1
			}
   #endif
#endif
}
...
#if  USE_HTK  ///播两声提示音之后启动语言识别
   #if TONE_AFTER_HTK_START
            psel_p->rpt_time = 1;
            psel_p->max_file_number = 2;
            user_val->ring_htk_start_flag=1;
            psel_p->file_name[1] = BPF_RING_MP3;
   #else
			psel_p->delay = 300;
...
{
#if BT_TWS 
   if(BT_CURRENT_CONN_PHONE==get_bt_current_conn_type())//当前只有手机连接上
   {

   }
   else if(BT_CURRENT_CONN_STEREO_MASTER==get_bt_current_conn_type())//当前主只连接从成功，没有手机连接
   {

   }
   else if(BT_CURRENT_CONN_STEREO_SALVE==get_bt_current_conn_type())//从机与主机连接成功
   {

   }
   else if(BT_CURRENT_CONN_STEREO_MASTER_PHONE==get_bt_current_conn_type())//当前连接的是手机，并且对箱从已经连接上
   {

   }
   else if(BT_CURRENT_CONN_STEREO_PHONE_MASTER==get_bt_current_conn_type())//当前主只连接从成功，同时手机已连接
   {
...
static void btstack_key_handler(void *ptr,int *msg)
{
	...
	switch(msg[0]){
        ...
        case MSG_BT_STEREO_SEARCH_DEVICE:
            ...
            if(BT_TWS_CHANNEL_SELECT&&BT_TWS_CHANNEL_SELECT==TWS_CHANNEL_RIGHT)//左耳为主,右耳不进行搜索配对
			{
...
void bt_rec_msg_deal(int *msg)
{
	...
	if(!get_sco_connect_status()&&(msg[0] == MSG_REC_START))//非通话按
    {
        return;
    }
