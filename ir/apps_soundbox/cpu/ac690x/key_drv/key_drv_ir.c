...
s32 ir_key_init(void)
{
    ...
    ret=timer_ctrl(timer1_hl,TIMER_CMD_IRFLT_SET,IR_PORTA(10),NULL);///IRFLT IO SEL
    if(ret!=TIMER_NO_ERR)
        return ret;
...
tu8 get_irkey_value(void)
{
    ...
    {
		put_u8hex((u8)ir_code.wData);
		put_u16hex((u16)ir_code.wUserCode);
		if((u8)ir_code.wData == 0x19)
		{
			IR_B_LED_OFF();
		}
		#if 0
        if (ir_code.wUserCode == 0x00ff)//0xFF00
        {
