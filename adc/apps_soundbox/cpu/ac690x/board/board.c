...
static void delay_ms(u32 n)
{
    while(n--)
    {
        delay(1000 * (SYS_Hz/1000000L));
    }
}
void  board_main(u32 cfg_addr, u32 addr, u32 res,u32 update_flag)
{
    ...
    JL_AUDIO->DAA_CON0 |= BIT(8) ;
    delay(20000 * (SYS_Hz/1000000L));
	irq_index_tab_reg((void *)irq_index_2_prio_tab, sizeof(irq_index_2_prio_tab));

	update_check(update_flag);

	set_bt_osc_internal_cfg(0x11,0x11);/*internal capacitor range:0x00~0x1F,时钟初始化前调用*/
