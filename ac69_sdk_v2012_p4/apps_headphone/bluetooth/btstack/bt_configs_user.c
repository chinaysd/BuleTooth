...
static void out_low_pwr_deal(u32 time_ms)
{
	...
	if(get_ldo5v_online_flag())                   //充电时不扫灯
		return;
...
static void bt_function_select_init()
{
...
#if BT_TWS 
	__set_stereo_scan_key(BT_TWS_SCAN_ENBLE);/*对箱没连接成功之前，通过按键开启可发现使能,即主从同时按下配对按键才进行配对*/ 
	...
	{
	    __set_stereo_role_disconnect_phone(1);/*从机已被手机连接，当主机来连接从机时，从机是否断开于手机的连接.为0时，从机会打断正在与主机连接的链路，做回主去反回连*/ 
        __set_stereo_device_channel(get_tws_channel_type());
	}
	__set_soundtrack_compound(1);    /*对箱左右声道分离使能*/
    __set_indicate_name_cmp_en(0);   /*对箱相同蓝牙名字不进行配对使能*/
...
#endif
	{
		__set_stereo_device_indicate("EH");/*设置对箱搜索标识,support两个字符标识，inquiry时候用,搜索到相应的标识才允许连接*/
	}
    ...
    __set_esco_repair_en(0);
	__set_pll_div_mode(1);/*0:integer_div,1:decimal_div*/
}
...
#if (BT_MODE != NORMAL_MODE)
	fcc_test_handle_register(fcc_uart_init,test_fcc_default_info);/*fcc?馐院⒉?*/
#endif
...
#else
	bt_stereo_register(NULL,NULL,NULL,NULL);  /*对箱接??*/
#endif
...
void bt_app_cfg()
{
	...
	ble_RF_test_uart_para_init(UART2_HARDWARE_NAME,9600,UART_TXPC4_RXPC5);
#endif
