...
static void set_outchannel0_pwm(u32 fre, u8 duty)
{
#ifdef USE_USB_IO_PWM
...
void led_scan(void *param)
{
	...
	if(get_ldo5v_online_flag())                   //³äµçÊ±²»É¨µÆ
		return;

	if(going_to_pwr_off || low_power_flag)         //¹Ø»ú»òÕßµÍµçµÄÊ±ºò²»É¨ÃèµÆ    
		return;
