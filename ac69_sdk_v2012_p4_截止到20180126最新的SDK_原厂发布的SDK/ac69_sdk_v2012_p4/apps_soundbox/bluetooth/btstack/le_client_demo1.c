...
bool ble_set_scan_param(u16 interval,u16 window)
{
    if((create_conn_flag) || (client_conn_handle != 0))
	{
		return FALSE;
	}
    client_cmd_ctrol(4, CLI_CMD_SCAN_PARAM, 1,interval,window);
	return TRUE;
}
...
void ble_client_send_test_key_num(u8 key_num)
{
	if(get_ble_test_key_flag())
	{
		...
		ble_user_send_test_key_num(client_conn_handle,key_num);
	}
}
